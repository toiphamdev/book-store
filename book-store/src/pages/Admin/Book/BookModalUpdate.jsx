...
const BookModalUpdate = ({ open, onClose, reloadTable, dataUpdate }) => {
  ...
  useEffect(() => {
    ...
    const sliderArr = slider.map((item) => {
      return {
        uid: uuidv4(),
        name: item,
        status: "done",
    ...
    const thumbnailArr = [
      {
        ...
        name: thumbnail,
        status: "done",
        url: `${import.meta.env.VITE_BASE_URL}/images/book/${thumbnail}`,
      },
    ];
    setDataThumbnail(thumbnailArr);
    setImageUrlThumbNail(thumbnailArr[0].url);
    ...
    form.setFieldsValue({
      ...
      quantity,
      sold,
      thumbnail: { fileList: thumbnailArr },
    });
    return () => {
      form.resetFields();
    };
    ...
    >
      ...
      >
        <Row style={{ display: "flex", justifyContent: "space-between" }}>
          ...
          <Form.Item style={{ display: "none" }} label="" name="_id">
            <Input />
          </Form.Item>
          <Col span={12}>
            <Form.Item
